...
def make_graph(days, moods):
    ...
    graph_screen.iconbitmap('img/moodcast_icon.ico')
    ...
    curvature = 2 if len(x) > 6 else 1
    z = np.polyfit(x, y, curvature)
    p = np.poly1d(z)
...
def make_bar_chart():
    bar_chart_screen = Tk()
    bar_chart_screen.title("Weekly report")
    bar_chart_screen.geometry("600x550")
    bar_chart_screen.resizable(False, False)
    bar_chart_screen.iconbitmap('img/moodcast_icon.ico')
    ...
    ax.set_yticks([0, 1, 2, 3, 4, 5])
    ax.set_yticklabels(("", "Awful", "Bad", "Meh", "Good", "Amazing"), fontsize=8.5)
    ax.title.set_text("My week")
    ax.set_xlabel("Day")

    # Plot bar chart
    ax.bar(x, y, color=acc_col)
...
def weekly_report():
    ...
    weekly_report_screen.resizable(False, False)
    ...
    # Banner
    Label(weekly_report_screen, text="", font=('Segoe UI Bold', 10), bg=acc_col, fg='white').pack(fill=X,
                                                                                                    anchor='center')

    # Calculate average mood
    avg_score = round(mean(weekly_scores)) - 1
    avg_mood = mood_options[avg_score]

    # Display average mood
    Label(weekly_report_screen, text=f"On average, you felt {avg_mood} this week", font=('Segoe UI Bold', 17),
          bg=bg_col,
          fg='white').place(relx=0.5, rely=0.07, anchor=CENTER)

    # Display encouraging comment
    comment = "You're doing great, keep it up!" if avg_score > 2 else "Keep your head up, next week will be awesome!"
    Label(weekly_report_screen, text=comment, font=('Segoe UI Bold', 12), bg=bg_col, fg='darkgrey').place(relx=0.5,
                                                                                                          rely=0.135,
                                                                                                          anchor=CENTER)

    # Bar chart button
    Button(weekly_report_screen, text="Show this week's progress", command=make_bar_chart,
           font=('Segoe UI Bold', 13), bg=acc_col, fg='white', padx=8, pady=8) \
    ...
    for count, mood in enumerate(weekly_moods):
        ...
        Label(weekly_report_screen, text=mood, font=('Segoe UI Bold', 20), bg=bg_col, fg=acc_col) \
...
# Tkinter elements
graph_btn = Button(root, text="Show progress", command=show_graph, font=font, bg=acc_col, fg='white', padx=8, pady=8) \
    .place(relx=0.5, rely=0.8, anchor=CENTER)
banner_lbl = Label(root, text="Moodcast", font=('Segoe UI Bold', 32), bg=acc_col, fg='white') \
...
day_lbl = Label(root, textvariable=current_day, font=('Segoe UI Bold', 18), bg=bg_col,
                fg=acc_col).pack(anchor='center')
text_lbl = Label(root, text="How are you feeling today?", font=('Segoe UI Bold', 26), bg=bg_col, fg='white').place(
